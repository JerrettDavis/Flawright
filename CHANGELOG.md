...
- **WPF ContextMenu (Popup HWND)**: `TestAppRightClickContextMenuTests.RightClickAsync_OpensContextMenu` and `RightClickThenClickMenuItem_DismissesMenu` are skipped. WPF `ContextMenu` items live in a separate top-level Popup HWND that is not a child of the main window's UIA subtree. `page.Locator("#menuEdit")` searches only the main window's descendants and cannot see the popup. The correct fix (`WaitForDialogAsync` to attach to the popup) is blocked by the popup having no window title on headless CI runners, causing `WaitForDialogAsync` to time out. Fix requires reliable owned-popup discovery (e.g., enumerate all owned HWNDs without a title filter).
- **`GetByRole(AriaRole.Tab)` on headless CI**: `TestAppTabControlTests.TabControl_GetByRole_ResolvesTabItem` and `TabControl_GetByRole_ClickActivatesTab` are skipped. `GetByRole(AriaRole.Tab)` generates a `ByControlType(ControlType.TabItem)` FlaUI condition which returns zero elements on headless Windows Server CI runners (GitHub Actions `windows-2025`), despite AutomationId-based TabItem selectors (`#tabInputs`, `#tabSelection`) working correctly. Suspected FlaUI UIA3 ControlType condition behavior difference on server runners without a compositor. Use AutomationId selectors with `SelectAsync()` as the production-safe alternative for CI.
- **WPF TreeView child realization on headless CI**: `TestAppAdvancedControlsTests.TreeView_Expand_ShowsChildren` and `TreeView_SelectChild_ChangesSelection` are skipped. Even with `VirtualizingPanel.IsVirtualizing="False"`, WPF lazy-realizes `TreeViewItem` children in the UIA tree on systems without an active compositor (GitHub Actions `windows-2025`). The `ExpandAsync()` production API works correctly — verified by local Win11 testing — but the child items don't surface in headless CI's UIA tree after expansion. No production code change; documented limitation. Use TreeView locally; expect skip on CI.
- **RealInputMode lacks per-monitor DPI compensation in CI-vs-local divergence**: Static `Flawright()` constructor sets `DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2` so UIA-reported coordinates align with SendInput's coordinate space on the target window's monitor. On user machines with non-100% DPI scaling or multi-monitor setups, this is necessary for click coordinates to land correctly. CI's single-monitor 100% DPI runner is unaffected either way.
